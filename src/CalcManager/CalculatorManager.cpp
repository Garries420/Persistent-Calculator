...
namespace CalculationManager
{
    ...
    {
        const auto currentMode = (m_pHistory == m_pSciHistory.get()) ? CalculatorMode::Scientific : CalculatorMode::Standard;
        SetHistoryItems(currentMode, historyItems);

        for (size_t index = 0; index < historyItems.size(); ++index)
        {
            OnHistoryItemAdded(static_cast<unsigned int>(index));
        }
    }

    void CalculatorManager::SetHistoryItems(
        _In_ CalculatorMode mode,
        _In_ std::vector<std::shared_ptr<HISTORYITEM>> const& historyItems)
    {
        auto history = (mode == CalculatorMode::Standard) ? m_pStdHistory : m_pSciHistory;
