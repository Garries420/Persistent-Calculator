...
namespace
{
    ...
    constexpr long long AUTO_REFRESH_INTERVAL = 3LL * 60 * 60 * 10000000;
    ...
    constexpr auto AUTO_REFRESH_ATTEMPT_TIMESTAMP_KEY = L"CURRENCY_CONVERTER_AUTO_REFRESH_ATTEMPT_TIMESTAMP";
    constexpr auto CACHE_LANGCODE_KEY = L"CURRENCY_CONVERTER_LANGCODE";
    constexpr auto CACHE_DELIMITER = L"%";

    constexpr auto PERSISTENT_DATA_FOLDER_NAME = L"Persistent Calculator";
    constexpr auto STATIC_DATA_FILENAME = L"Currency Metadata.json";
    constexpr auto ALL_RATIOS_DATA_FILENAME = L"Currency Rates.json";
    constexpr auto ISO_CODE_KEY = L"iso_code";
    constexpr auto CURRENCY_NAME_KEY = L"name";
    constexpr auto CURRENCY_SYMBOL_KEY = L"symbol";
    constexpr auto RATE_KEY = L"rate";
    constexpr auto BASE_CURRENCY_CODE_KEY = L"base";
    constexpr auto QUOTE_CURRENCY_CODE_KEY = L"quote";
    ...
    task<StorageFolder ^> GetPersistentDataFolderAsync()
    {
#ifdef VIEWMODEL_FOR_UT
        // Unit tests run in an isolated app-data container and must not touch
        // the developer's real Documents library.
        co_return ApplicationData::Current->LocalCacheFolder;
#else
        ...
        co_return co_await documents->CreateFolderAsync(
            StringReference(PERSISTENT_DATA_FOLDER_NAME),
            CreationCollisionOption::OpenIfExists);
#endif
    }

    bool IsWeekendInCentralEurope(DateTime currentTime)
    {
        Calendar ^ calendar = ref new Calendar();
        try
        {
            calendar->ChangeTimeZone("Europe/Stockholm");
        }
        catch (...)
        {
            // UTC is a conservative fallback near the weekend boundary if a
            // particular Windows image does not expose the IANA zone database.
            calendar->ChangeTimeZone("UTC");
        }
        calendar->SetDateTime(currentTime);
        return calendar->DayOfWeek == DayOfWeek::Saturday || calendar->DayOfWeek == DayOfWeek::Sunday;
    }
...
namespace CalculatorApp
{
    namespace ViewModel::DataLoaders
    {
        ...
        namespace CurrencyDataLoaderConstants
        {
            ...
            StringReference AutoRefreshAttemptTimestampKey(AUTO_REFRESH_ATTEMPT_TIMESTAMP_KEY);
            ...
            long long AutoRefreshInterval = AUTO_REFRESH_INTERVAL;
        }
    }
}
...
CurrencyDataLoader::CurrencyDataLoader(const wchar_t* forcedResponseLanguage)
    ...
    , m_skipAutomaticWebAttempt(false)
{
        ...
        create_task(
            [this]() -> task<bool>
            {
                vector<function<task<bool>()>> loadFunctions = {
                    ...
                    [this]()
                    {
                        return m_skipAutomaticWebAttempt ? task_from_result(false) : TryLoadDataFromWebAsync();
                    },
                };
...
bool CurrencyDataLoader::ShouldAutomaticallyRefresh(DateTime cacheTimestamp, DateTime currentTime)
{
    if (cacheTimestamp.UniversalTime <= 0 || currentTime.UniversalTime <= cacheTimestamp.UniversalTime)
    {
        return false;
    }

    if (currentTime.UniversalTime - cacheTimestamp.UniversalTime < AUTO_REFRESH_INTERVAL)
    {
        return false;
    }

#ifdef VIEWMODEL_FOR_UT
    // Unit tests use an isolated cache and must not depend on the day on which
    // the test happens to run. Production applies the CET/CEST weekend rule.
    return true;
#else
    return !IsWeekendInCentralEurope(currentTime);
#endif
}
...
task<bool> CurrencyDataLoader::TryLoadDataFromCacheAsync()
{
    try
    {
        ...
        {
            if (localSettings != nullptr && localSettings->Values->HasKey(AutoRefreshAttemptTimestampKey))
            {
                DateTime lastAttempt = static_cast<DateTime>(localSettings->Values->Lookup(AutoRefreshAttemptTimestampKey));
                DateTime currentTime = Utils::GetUniversalSystemTime();
                m_skipAutomaticWebAttempt = !ShouldAutomaticallyRefresh(lastAttempt, currentTime);
            }
            co_return false;
        }
        ...
        DateTime refreshReferenceTimestamp = m_cacheTimestamp;
        if (localSettings->Values->HasKey(AutoRefreshAttemptTimestampKey))
        {
            DateTime lastAttempt = static_cast<DateTime>(localSettings->Values->Lookup(AutoRefreshAttemptTimestampKey));
            if (lastAttempt.UniversalTime > refreshReferenceTimestamp.UniversalTime)
            {
                refreshReferenceTimestamp = lastAttempt;
            }
        }
        DateTime currentTime = Utils::GetUniversalSystemTime();
        if (ShouldAutomaticallyRefresh(refreshReferenceTimestamp, currentTime) && m_networkAccessBehavior == NetworkAccessBehavior::Normal)
        {
...
task<bool> CurrencyDataLoader::TryFinishLoadFromCacheAsync()
{
    ...
    StorageFolder ^ localCacheFolder = co_await GetPersistentDataFolderAsync();
...
task<bool> CurrencyDataLoader::TryLoadDataFromWebAsync()
{
    try
    {
        ...
        if (localSettings != nullptr)
        {
            localSettings->Values->Insert(AutoRefreshAttemptTimestampKey, Utils::GetUniversalSystemTime());
        }
...
bool CurrencyDataLoader::TryParseStaticData(_In_ String ^ rawJson, _Inout_ vector<UCM::CurrencyStaticData>& staticData)
{
    ...
    staticData.clear();
    staticData.reserve(size_t{ data->Size });
    for (unsigned int i = 0; i < data->Size; i++)
    {
        ...
        String ^ currencyCodeValue = obj->GetNamedString(StringReference(ISO_CODE_KEY));
        String ^ currencyNameValue = obj->GetNamedString(StringReference(CURRENCY_NAME_KEY));
        String ^ currencySymbolValue = obj->GetNamedString(StringReference(CURRENCY_SYMBOL_KEY));
        if (currencyCodeValue == nullptr || currencyCodeValue->Length() != 3 || currencyNameValue == nullptr || currencyNameValue->IsEmpty())
        {
            continue;
        }

        const wstring currencyCode{ currencyCodeValue->Data() };
        const wstring currencyName{ currencyNameValue->Data() };
        const wstring currencySymbol = (currencySymbolValue == nullptr) ? currencyCode : wstring{ currencySymbolValue->Data() };

        // Frankfurter provides canonical currency names and symbols but not a
        // separate country label. The ISO code is the compact leading label;
        // the canonical name remains the accessible currency name.
        staticData.push_back(CurrencyStaticData{ currencyCode, currencyCode, currencyCode, currencyName, currencySymbol });
    }
...
bool CurrencyDataLoader::TryParseAllRatiosData(_In_ String ^ rawJson, _Inout_ CurrencyRatioMap& allRatios)
{
    ...
    for (unsigned int i = 0; i < data->Size; i++)
    {
        ...
        double relativeRatio = obj->GetNamedNumber(StringReference(RATE_KEY));
        wstring targetCurrencyCode = obj->GetNamedString(StringReference(QUOTE_CURRENCY_CODE_KEY))->Data();
        sourceCurrencyCode = obj->GetNamedString(StringReference(BASE_CURRENCY_CODE_KEY))->Data();

        if (relativeRatio > 0.0 && !targetCurrencyCode.empty())
        {
            allRatios.emplace(targetCurrencyCode, CurrencyRatio{ relativeRatio, sourceCurrencyCode, targetCurrencyCode });
        }
    }

    // Microsoft's ratio model expects an identity ratio for the base. Assign it
    // explicitly whether or not the current endpoint response includes one.
    allRatios[sourceCurrencyCode] = CurrencyRatio{ 1.0, sourceCurrencyCode, sourceCurrencyCode };

    return allRatios.size() > 1;
}
