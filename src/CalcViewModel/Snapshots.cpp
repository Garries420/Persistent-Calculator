...
namespace CalculatorApp::ViewModel::Snapshot
{
    ...
    CalcManagerSnapshot::CalcManagerSnapshot()
    {
        ...
        StandardHistoryItems = nullptr;
        ScientificHistoryItems = nullptr;
    }
    ...
    CalcManagerSnapshot::CalcManagerSnapshot(const CalculationManager::CalculatorManager& calcMgr)
    {
        ...
        auto appendHistory = [](const std::vector<std::shared_ptr<CalculationManager::HISTORYITEM>>& items)
        {
            auto snapshots = ref new Platform::Collections::Vector<CalcManagerHistoryItem ^>();
            for (auto& item : items)
            {
                snapshots->Append(ref new CalcManagerHistoryItem(*item));
            }
            return snapshots;
        };

        StandardHistoryItems = appendHistory(calcMgr.GetHistoryItems(CalculationManager::CalculatorMode::Standard));
        ScientificHistoryItems = appendHistory(calcMgr.GetHistoryItems(CalculationManager::CalculatorMode::Scientific));
    }
