...
StandardCalculatorViewModel::StandardCalculatorViewModel()
    ...
    , m_PersistentExpression(L"")
    , m_PersistentHistorySessionId(0)
    , m_persistentLastCommandWasEquals(false)
...
void StandardCalculatorViewModel::OnButtonPressed(Object ^ parameter)
{
    ...
    if (IsInError)
    {
        ...
        ResetPersistentExpressionSession();
    ...
    else
    {
        ...
        else
        {
            ...
            String ^ previousDisplayValue = DisplayValue;
            const wstring previousEngineExpression = GetExpressionText();
            TraceLogger::GetInstance()->UpdateButtonUsage(numOpEnum, GetCalculatorMode());
            m_standardCalculatorManager.SendCommand(cmdenum);
            UpdatePersistentExpression(numOpEnum, previousDisplayValue, previousEngineExpression);
        }
    }
}

wstring StandardCalculatorViewModel::GetExpressionText() const
{
    wstring expression;
    for (auto&& token : m_ExpressionTokens)
    {
        expression += token->Token->Data();
    }

    while (!expression.empty() && iswspace(expression.back()))
    {
        expression.pop_back();
    }
    return expression;
...
void StandardCalculatorViewModel::ResetPersistentExpressionSession()
{
    m_persistentExpressionPrefix.clear();
    m_persistentCurrentOperationPrefix.clear();
    m_persistentLastCommandWasEquals = false;
    PersistentExpression = L"";
    PersistentHistorySessionId = PersistentHistorySessionId + 1;
}

bool StandardCalculatorViewModel::IsPersistentBinaryCommand(NumbersAndOperatorsEnum command)
{
    switch (command)
    {
    ...
    case NumbersAndOperatorsEnum::Mod:
    case NumbersAndOperatorsEnum::XPowerY:
    case NumbersAndOperatorsEnum::YRootX:
    case NumbersAndOperatorsEnum::LogBaseY:
    case NumbersAndOperatorsEnum::And:
    case NumbersAndOperatorsEnum::Nand:
    case NumbersAndOperatorsEnum::Or:
    case NumbersAndOperatorsEnum::Nor:
    case NumbersAndOperatorsEnum::Xor:
    case NumbersAndOperatorsEnum::Lsh:
    case NumbersAndOperatorsEnum::Rsh:
        return true;
    default:
        return false;
    }
...
wstring StandardCalculatorViewModel::GetPersistentBinaryOperatorSymbol(NumbersAndOperatorsEnum command)
{
    switch (command)
    {
    case NumbersAndOperatorsEnum::Add:
...
void StandardCalculatorViewModel::UpdatePersistentExpression(
    NumbersAndOperatorsEnum command,
    String ^ previousDisplayValue,
    wstring const& previousEngineExpression)
{
    const bool isModeChange = command == NumbersAndOperatorsEnum::IsStandardMode
        || command == NumbersAndOperatorsEnum::IsScientificMode
        || command == NumbersAndOperatorsEnum::IsProgrammerMode;
    if (command == NumbersAndOperatorsEnum::Clear || isModeChange)
    {
        ResetPersistentExpressionSession();
        return;
    }

    const bool isDigitLike = (command >= NumbersAndOperatorsEnum::Zero && command <= NumbersAndOperatorsEnum::Nine)
        || (command >= NumbersAndOperatorsEnum::A && command <= NumbersAndOperatorsEnum::F)
        || command == NumbersAndOperatorsEnum::Decimal
        || command == NumbersAndOperatorsEnum::Backspace
        || command == NumbersAndOperatorsEnum::Exp;

    wstring currentExpression = GetExpressionText();
    if (currentExpression.empty())
    {
        currentExpression = DisplayValue == nullptr ? L"" : DisplayValue->Data();
    }
    else if (isDigitLike && DisplayValue != nullptr && !DisplayValue->IsEmpty())
    {
        const wstring displayValue = DisplayValue->Data();
        const bool alreadyEndsWithDisplay = currentExpression.length() >= displayValue.length()
            && currentExpression.compare(
                currentExpression.length() - displayValue.length(),
                displayValue.length(),
                displayValue) == 0;
        if (!alreadyEndsWithDisplay)
        {
            currentExpression += L" ";
    ...
    const wstring previousExpression = PersistentExpression == nullptr
        ? L""
        : PersistentExpression->Data();
    const wstring previousDisplay = previousDisplayValue == nullptr
        ? L""
        : previousDisplayValue->Data();
    wstring previousEngine = previousEngineExpression;
    if (!previousDisplay.empty()
        && (previousEngine.empty()
            || previousEngine.length() < previousDisplay.length()
            || previousEngine.compare(previousEngine.length() - previousDisplay.length(), previousDisplay.length(), previousDisplay) != 0))
    {
        if (!previousEngine.empty() && !iswspace(previousEngine.back()))
        {
            previousEngine += L" ";
        }
        previousEngine += previousDisplay;
    }
    const bool previousEndsWithDisplay = !previousDisplay.empty()
        && previousExpression.length() >= previousDisplay.length()
        && previousExpression.compare(
            previousExpression.length() - previousDisplay.length(),
            previousDisplay.length(),
            previousDisplay) == 0;
    if (command == NumbersAndOperatorsEnum::Equals && !previousExpression.empty())
    {
        m_persistentExpressionPrefix = previousExpression + L" =";
        m_persistentCurrentOperationPrefix.clear();
        PersistentExpression = ref new String(m_persistentExpressionPrefix.c_str());
        m_persistentLastCommandWasEquals = true;
        return;
    }

    if (m_persistentLastCommandWasEquals && isDigitLike)
    {
        // Persistent Calculator deliberately keeps one draft until C/Escape.
        // Starting a fresh operand after '=' therefore extends that same saved
        // session instead of silently creating a separate history card.
        m_persistentExpressionPrefix = previousExpression;
        m_persistentCurrentOperationPrefix.clear();
        currentExpression = DisplayValue == nullptr ? L"" : DisplayValue->Data();
    }
    else if (IsPersistentBinaryCommand(command)
        && previousEndsWithDisplay
        && previousExpression != previousDisplay)
    {
        m_persistentExpressionPrefix = previousExpression + L" =";

        wstring operatorSuffix;
        const size_t previousDisplayPosition = previousDisplay.empty()
            ? wstring::npos
            : currentExpression.rfind(previousDisplay);
        if (previousDisplayPosition != wstring::npos)
        {
            operatorSuffix = currentExpression.substr(previousDisplayPosition + previousDisplay.length());
        }
        else if (currentExpression.length() >= previousEngine.length()
            && currentExpression.compare(0, previousEngine.length(), previousEngine) == 0)
        {
            operatorSuffix = currentExpression.substr(previousEngine.length());
        }
        while (!operatorSuffix.empty() && iswspace(operatorSuffix.front()))
        {
            operatorSuffix.erase(operatorSuffix.begin());
        }
        if (operatorSuffix.empty())
        {
            operatorSuffix = GetPersistentBinaryOperatorSymbol(command);
        }

        m_persistentCurrentOperationPrefix = DisplayValue == nullptr ? L"" : DisplayValue->Data();
        if (!operatorSuffix.empty())
        {
            m_persistentCurrentOperationPrefix += L" ";
            m_persistentCurrentOperationPrefix += operatorSuffix;
        }
        currentExpression = m_persistentCurrentOperationPrefix;
    }
    else if (m_persistentLastCommandWasEquals && IsPersistentBinaryCommand(command))
    {
        m_persistentExpressionPrefix = previousExpression;
        m_persistentCurrentOperationPrefix = currentExpression;
    }
    else if (!m_persistentCurrentOperationPrefix.empty() && isDigitLike)
    {
        currentExpression = m_persistentCurrentOperationPrefix;
        if (DisplayValue != nullptr && !DisplayValue->IsEmpty())
        {
            currentExpression += L" ";
            currentExpression += DisplayValue->Data();
        }
    }

    wstring combined;
    if (!m_persistentExpressionPrefix.empty())
    {
        combined = m_persistentExpressionPrefix;
        if (!currentExpression.empty())
        {
            combined += L" ";
            combined += currentExpression;
        }
    }
    else
    {
    ...
    PersistentExpression = ref new String(combined.c_str());
...
CalculatorApp::ViewModel::Snapshot::StandardCalculatorSnapshot ^ StandardCalculatorViewModel::Snapshot::get()
{
    ...
    result->CalcManager = HistorySnapshot;
...
void StandardCalculatorViewModel::RecallPersistentHistory(String ^ expression, String ^ result)
{
    ...
    ResetPersistentExpressionSession();
    OnPaste(result);
    const wstring recalledExpression = expression->Data();
    PersistentExpression = expression;
    m_persistentExpressionPrefix = recalledExpression;
    m_persistentCurrentOperationPrefix.clear();
    m_persistentLastCommandWasEquals = !recalledExpression.empty()
        && recalledExpression.back() == L'=';
}

CalculatorApp::ViewModel::Snapshot::CalcManagerSnapshot ^ StandardCalculatorViewModel::HistorySnapshot::get()
{
    return ref new CalculatorApp::ViewModel::Snapshot::CalcManagerSnapshot(m_standardCalculatorManager);
}
...
void CalculatorApp::ViewModel::StandardCalculatorViewModel::Snapshot::set(CalculatorApp::ViewModel::Snapshot::StandardCalculatorSnapshot ^ snapshot)
{
    ...
    RestoreHistory(snapshot->CalcManager);
...
void CalculatorApp::ViewModel::StandardCalculatorViewModel::RestoreHistory(
    CalculatorApp::ViewModel::Snapshot::CalcManagerSnapshot ^ snapshot)
{
    if (snapshot == nullptr)
    {
        return;
    ...
    if (snapshot->StandardHistoryItems != nullptr || snapshot->ScientificHistoryItems != nullptr)
    {
        if (snapshot->StandardHistoryItems != nullptr)
        {
            m_standardCalculatorManager.SetHistoryItems(
                CalculationManager::CalculatorMode::Standard,
                ToUnderlying(snapshot->StandardHistoryItems));
        }
        if (snapshot->ScientificHistoryItems != nullptr)
        {
            m_standardCalculatorManager.SetHistoryItems(
                CalculationManager::CalculatorMode::Scientific,
                ToUnderlying(snapshot->ScientificHistoryItems));
        }
    }
    else if (snapshot->HistoryItems != nullptr)
    {
        // Backward compatibility with Calculator snapshots created before
        // Persistent Calculator stored Standard and Scientific history separately.
        m_standardCalculatorManager.SetHistoryItems(ToUnderlying(snapshot->HistoryItems));
    }
}
