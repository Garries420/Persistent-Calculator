...
void UnitConverterViewModel::RestorePersistentHistoryValue(String ^ unlocalizedValue)
{
    if (unlocalizedValue == nullptr || unlocalizedValue->IsEmpty())
    {
        return;
    }

    // Converter internals keep a culture-independent decimal point, while
    // OnPaste intentionally accepts only the user's configured separator.
    // Translate the persisted canonical value before replaying it so currency
    // grouping and decimal characters can never change the number on recall.
    wstring localizedValue(unlocalizedValue->Data());
    replace(localizedValue.begin(), localizedValue.end(), L'.', m_decimalSeparator);
    OnPaste(ref new String(localizedValue.c_str()));
}
