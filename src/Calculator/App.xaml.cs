...
namespace CalculatorApp
{
    ...
    {
        private const string PreviousSessionEndedCleanlyKey = "PreviousSessionEndedCleanly";
        ...
        private void OnAppLaunch(IActivatedEventArgs args, object arguments, bool isPreLaunch)
        {
            ...
            bool isNewWindowSession = Window.Current.Content == null;
            bool previousSessionEndedUncleanly = isNewWindowSession
                && localSettings.Values.ContainsKey(PreviousSessionEndedCleanlyKey)
                && localSettings.Values[PreviousSessionEndedCleanlyKey] is bool endedCleanly
                && !endedCleanly;
            ...
            else if (previousSessionEndedUncleanly)
            {
                // Windows can retain the oversized bounds produced by a XAML
                // failure. Recover only after an unclean exit so ordinary users'
                // manually chosen window sizes continue to be respected.
            ...
            {
                localSettings.Values[PreviousSessionEndedCleanlyKey] = false;
            }
        ...
        {
            ApplicationData.Current.LocalSettings.Values[PreviousSessionEndedCleanlyKey] = true;
            TraceLogger.GetInstance().LogButtonUsage();
        }

        private void OnResuming(object sender, object args)
        {
            ApplicationData.Current.LocalSettings.Values[PreviousSessionEndedCleanlyKey] = false;
        }
