...
namespace CalculatorApp.JsonUtils
{
    ...
    internal class CalcManagerSnapshotAlias
    {
        ...
        [JsonPropertyName("hs")]
        public IEnumerable<CalcManagerHistoryItemAlias> StandardHistoryItems
        {
            get => Value.StandardHistoryItems?.Select(x => new CalcManagerHistoryItemAlias { Value = x });
            set => Value.StandardHistoryItems = MapHistoryItems(value);
        }

        [JsonPropertyName("hc")]
        public IEnumerable<CalcManagerHistoryItemAlias> ScientificHistoryItems
        {
            get => Value.ScientificHistoryItems?.Select(x => new CalcManagerHistoryItemAlias { Value = x });
            set => Value.ScientificHistoryItems = MapHistoryItems(value);
        }

        private static IList<CalcManagerHistoryItem> MapHistoryItems(IEnumerable<CalcManagerHistoryItemAlias> value)
        {
            return value?.Select(x => new CalcManagerHistoryItem
            {
                Tokens = x.Tokens.Select(Helpers.MapToken).ToList(),
                Commands = x.Commands.Select(Helpers.MapCommandAlias).ToList(),
                Expression = x.Expression,
