...
namespace CalculatorUITestFramework
{
    public class WinAppDriverLocalServer : IDisposable
    {
        ...
        public WinAppDriverLocalServer()
        {
            ...
            string path = Environment.GetEnvironmentVariable("WINAPPDRIVER_PATH");
            if (string.IsNullOrWhiteSpace(path) || !File.Exists(path))
            {
                path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), driverRelativePath);
            }
            if (!File.Exists(path))
            {
