...
namespace CalculatorUnitTests
{
    ...
    public:
        TEST_METHOD_INITIALIZE(DeleteCacheFiles)
        {
            ...
            RemoveFromLocalSettings(CurrencyDataLoaderConstants::AutoRefreshAttemptTimestampKey);
        }
...
TEST_METHOD(Load_NoCache_SuppressesRecentAutomaticRetry)
{
    RemoveFromLocalSettings(CurrencyDataLoaderConstants::CacheTimestampKey);
    InsertToLocalSettings(
        CurrencyDataLoaderConstants::AutoRefreshAttemptTimestampKey,
        Utils::GetUniversalSystemTime());
    CurrencyDataLoader loader{ L"en-US" };

    auto dataLoadedEvent = task_completion_event<void>();
    loader.SetViewModelCallback(std::make_shared<DataLoadedCallback>(dataLoadedEvent));
    auto dataLoadedTask = create_task(dataLoadedEvent);
    loader.LoadData();
    dataLoadedTask.wait();
...
TEST_METHOD(AutomaticRefreshPolicy_UsesThreeHourMinimum)
{
    DateTime now = Utils::GetUniversalSystemTime();
    DateTime recent;
    recent.UniversalTime = now.UniversalTime - CurrencyDataLoaderConstants::AutoRefreshInterval + 1;
    DateTime stale;
    stale.UniversalTime = now.UniversalTime - CurrencyDataLoaderConstants::AutoRefreshInterval;
    DateTime future;
    future.UniversalTime = now.UniversalTime + 1;

    VERIFY_IS_FALSE(CurrencyDataLoader::ShouldAutomaticallyRefresh(recent, now));
    VERIFY_IS_TRUE(CurrencyDataLoader::ShouldAutomaticallyRefresh(stale, now));
    VERIFY_IS_FALSE(CurrencyDataLoader::ShouldAutomaticallyRefresh(future, now));
}
...
TEST_CLASS(CurrencyConverterUnitTests)
{
    ...
    TEST_METHOD(Loaded_LoadOrderedUnits)
    {
        ...
        VERIFY_ARE_EQUAL(std::wstring(L"USD - United States Dollar"), usdUnit.name);
        VERIFY_ARE_EQUAL(std::wstring(L"USD"), usdUnit.abbreviation);

        VERIFY_ARE_EQUAL(std::wstring(L"EUR - Euro"), eurUnit.name);
    ...
    TEST_METHOD(Loaded_GetCurrencyRatioEquality_Valid)
    {
        ...
        VERIFY_ARE_EQUAL(std::wstring(L"1 USD United States Dollar = 0.9205 EUR Euro"), ratio.second);
    }
