...
{
    void VerifyPersistentHistoryState(
        StandardCalculatorViewModel ^ viewModel,
        int expectedSession,
        const wchar_t* expectedExpression,
        const wchar_t* checkpoint)
    {
        const std::wstring actualExpression = viewModel->PersistentExpression == nullptr
            ? L""
            : viewModel->PersistentExpression->Data();
        if (viewModel->PersistentHistorySessionId == expectedSession && actualExpression == expectedExpression)
        {
            return;
        }

        std::wstring message(checkpoint);
        message += L" | expected session=";
        message += std::to_wstring(expectedSession);
        message += L", actual session=";
        message += std::to_wstring(viewModel->PersistentHistorySessionId);
        message += L" | expected expression=\"";
        message += expectedExpression;
        message += L"\", actual expression=\"";
        message += actualExpression;
        message += L"\" | result=\"";
        message += viewModel->DisplayValue == nullptr ? L"<null>" : viewModel->DisplayValue->Data();
        message += L"\"";
    ...
    public:
        ...
        TEST_METHOD(PersistentExpressionKeepsIntermediateResultsInOneChainTest)
        {
            m_viewModel->ButtonPressed->Execute(NumbersAndOperatorsEnum::Clear);
            const int session = m_viewModel->PersistentHistorySessionId;
            m_viewModel->ButtonPressed->Execute(NumbersAndOperatorsEnum::Eight);
            m_viewModel->ButtonPressed->Execute(
                ref new CalculatorButtonPressedEventArgs(L"divided by", NumbersAndOperatorsEnum::Divide));
            m_viewModel->ButtonPressed->Execute(NumbersAndOperatorsEnum::Two);
            m_viewModel->ButtonPressed->Execute(
                ref new CalculatorButtonPressedEventArgs(L"times", NumbersAndOperatorsEnum::Multiply));

            VerifyPersistentHistoryState(m_viewModel, session, L"8 \u00f7 2 = 4 \u00d7", L"after 8 / 2 *");

            m_viewModel->ButtonPressed->Execute(NumbersAndOperatorsEnum::Three);
            m_viewModel->ButtonPressed->Execute(NumbersAndOperatorsEnum::Equals);

            VerifyPersistentHistoryState(m_viewModel, session, L"8 \u00f7 2 = 4 \u00d7 3 =", L"after 8 / 2 * 3 =");
            VERIFY_ARE_EQUAL(StringReference(L"12"), m_viewModel->DisplayValue);
        }

        TEST_METHOD(PersistentExpressionKeepsSessionAcrossEqualsAndStartsNewOnClearTest)
        {
            const int startingSession = m_viewModel->PersistentHistorySessionId;
            m_viewModel->ButtonPressed->Execute(NumbersAndOperatorsEnum::One);
            m_viewModel->ButtonPressed->Execute(NumbersAndOperatorsEnum::Equals);
            m_viewModel->ButtonPressed->Execute(NumbersAndOperatorsEnum::Two);

            VerifyPersistentHistoryState(m_viewModel, startingSession, L"1 = 2", L"after 1 = 2");

            m_viewModel->ButtonPressed->Execute(NumbersAndOperatorsEnum::Clear);
            VerifyPersistentHistoryState(m_viewModel, startingSession + 1, L"", L"after explicit clear");
        }

        TEST_METHOD(PersistentExpressionStartsNewHistorySessionOnModeChangeTest)
        {
            m_viewModel->ButtonPressed->Execute(NumbersAndOperatorsEnum::Nine);
            const int standardSession = m_viewModel->PersistentHistorySessionId;

            m_viewModel->SetCalculatorType(ViewMode::Scientific);

            VerifyPersistentHistoryState(
                m_viewModel,
                standardSession + 1,
                L"",
                L"after Standard to Scientific mode change");
        }
