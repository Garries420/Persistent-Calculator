...
TEST_METHOD(TestRestorePersistentHistoryValueUsesConfiguredDecimalSeparator)
{
    shared_ptr<UnitConverterMock> mock = make_shared<UnitConverterMock>();
    VM::UnitConverterViewModel vm(mock);

    UINT callCount = mock->m_sendCommandCallCount;
    vm.RestorePersistentHistoryValue("5.69");

    // Clear, 5, decimal, 6, and 9 must all reach the converter even when the
    // user's configured decimal separator is not a period.
    callCount += 5;
    VERIFY_ARE_EQUAL(callCount, mock->m_sendCommandCallCount);
    VERIFY_IS_TRUE(UCM::Command::Nine == mock->m_lastCommand);
}
